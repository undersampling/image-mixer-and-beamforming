...
export function ScenarioLoader() {
  const { scenarios, currentScenario, loadScenario, saveScenario, resetScenario } = useSimulator();
  ...
  return (
    <Card title="📁 Scenario">
      <Dropdown
        ...
        placeholder={scenarios.length === 0 ? "Loading scenarios..." : "Select a scenario..."}
      />
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
        <Button variant="primary" onClick={handleSave} disabled={!currentScenario}>
          Save
        </Button>
        <Button variant="secondary" onClick={handleReset} disabled={!currentScenario}>
          Reset Default
        </Button>
      </div>
    </Card>
